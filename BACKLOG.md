...
### Correction: `EMBED_CHAR_CAP` is semantically decorative but computationally load-bearing

The entry above titled "`EMBED_CHAR_CAP` is decorative and the real cap is four times smaller"
is half wrong, and the wrong half matters. `_encode` wordpieces the *entire* input and only
truncates at `return`, so the char cap does nothing to the vector and a great deal to the work.
Measured on `project_desktoppet.md` (211,644 chars): **87.6 ms uncapped, 3.2 ms at 8000, 0.7 ms
at 2000, all three producing identical 256 ids.**

The fix is a `break` once `len(ids) >= 256`: provably vector-identical, no `EMBED_ID` bump, pays
on every `--rebuild`. Do **not** instead tighten the cap to 2000, which looks equivalent and is
not: a file of long unsplittable tokens can yield fewer than 256 ids from 2000 chars, and that
would change vectors.
...
### `rank()` returns two fields nobody reads

`rank_vec` and `rank_lex` are consumed by neither `search()` nor `gate_recall.measure()`, and
producing them costs two full sorts per query (measured 0.03 ms, so this is dead API surface
rather than a performance problem). Either delete them, or say in the docstring that they exist
for ad-hoc inspection. Left because a bench comparing fusion strategies would want exactly them.

### `save_index`'s temp file has a fixed name

`INDEX_PATH + ".tmp"`. The `os.replace` is atomic, but two concurrent writers interleave into
the same temp path first, and the extension's `autoSyncRecallIfStale` runs on a 15-minute timer
against the same store a CLI query may be updating. A pid suffix closes it. Narrow, non-zero.

### `--lexical-only` drops the description line

In lexical mode `idx["files"]` is empty, so `files.get(n, {}).get("desc", "")` returns `""` and
